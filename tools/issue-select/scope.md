...
I'm comfortable working across the stack: Python backend work (FastAPI
services, APIs, RAG/ingestion pipelines) and React/TypeScript frontend
work. I'm looking for broad first-contribution experience rather than
sticking to one layer, so I don't have a strong preference between
backend and frontend issues. I want to get better at reading an
unfamiliar codebase quickly and writing changes that fit its existing
patterns. Nothing in particular to avoid; I'd rather start with a
smaller, well-scoped bug than an open-ended feature for my first issue.
